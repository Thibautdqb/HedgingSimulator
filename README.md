...
```mermaid
graph TD
    A[Stratégie de Couverture] --> B[1. Chargement des Données]
    B --> B1[Actifs]
    B --> B2[Options]
    B --> B3[Informations]
    B --> B4[Portefeuille]
    B4 --> B4_1["Positions actuelles"]

    A --> C[2. Calcul de l'Exposition]
    C --> C1[Exposition totale]
    C1 --> C1_1["Somme des positions"]
    C --> C2[Exposition partielle]
    C2 --> C2_1["Calcul par StockID"]

    C --> D[Détails]
    D --> D1[Exposition par action]
    D1 --> D1_1["Valeur des positions"]
    D1_1 --> D1_2["position_quantity * current_price"]
    D1 --> D1_3["Type de couverture"]
    D1_3 --> D1_4["Ajustée (delta, gamma)"]
    ...
    A --> H[4. Optimisation]
    H --> H1["Minimiser grecs"]
    H1 --> H1_1[Delta]
    H1 --> H1_2[Gamma]

    A --> J[5. Résultats]
    J --> J1[Quantité optimale]
```
